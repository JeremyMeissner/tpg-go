...
foreach ($array["disruptions"] as $key => $value) {
  ...
  if (selectIncident($value["timestamp"], $value["consequence"])) {
    ...
    echo "EXISTE PAS -";
    insertIncident($value["timestamp"], $value["place"], $value["nature"], $value["consequence"]);
    insertLine($value["lineCode"], selectIncident($value["timestamp"], $value["consequence"])["Id_Incident"]);
  }
}
...
function insertIncident($date, $place, $nature, $consequence) {
  $sql = "INSERT INTO `INCIDENT`(`Dttm_Incident`, `Geo_Place`, `Txt_Nature`, `Txt_Result`)
  VALUES (:date, :place, :nature, :consequence)";
  $query = connect()->prepare($sql);
  $query->execute([
    ':date' => $date,
    ':place' => $place,
...
function insertLine($lineCode, $incident) {
  $sql = "INSERT INTO `LINE`(`Cd_Line`, `Id_Incident`)
  VALUES (:lineCode, :incident)";
  $query = connect()->prepare($sql);
  $query->execute([
    ':lineCode' => $lineCode,
    ':incident' => $incident
